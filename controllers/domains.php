...
/**
 * Domain summary report controller.
 *
 * @category   Apps
...
class Domains extends ClearOS_Controller
{
    ...
    function _report($type)
    {
        ...
        try {
            $data['data'] = $this->proxy_report->get_domain_data('FIXME', '');
            $data['ranges'] = $this->proxy_report->get_date_ranges();

            $data['type'] = 'domains';

            $data['key'] = lang('proxy_report_domain');
            $data['value'] = lang('proxy_report_hits');
        ...
        if ($type === 'simple') {
            $this->page->view_form('proxy_report/simple', $data, lang('proxy_report_domains'));
        } else {
            $options['type'] = MY_Page::TYPE_REPORT;
            $this->page->view_form('proxy_report/full', $data, lang('proxy_report_domains'), $options);
        }
    }
    ...
    function get_data($date_range, $summary_index = 10)
    {
        ...
        try {
            $data = $this->proxy_report->get_domain_data($date_range, $summary_index);
        } catch (Exception $e) {
